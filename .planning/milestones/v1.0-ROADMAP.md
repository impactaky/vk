...
This milestone fixed the broken follow-up command and aligned the CLI with the current vibe-kanban backend API. The work progressed from critical bug fixes (follow-up endpoint), through schema corrections (multi-repo support), type alignment (API parity), and session command exposure.
