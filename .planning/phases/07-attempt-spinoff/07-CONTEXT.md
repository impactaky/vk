...
## Summary
Create a child task that inherits context from the current workspace. The command spawns a new task linked to the current workspace's task as its parent, and sends an initial message to the new agent.
...
### Title Input Flow
- **Title and message are separate** — title is not automatically derived from message
...
- **No validation** — no character limits or special handling for empty/long titles
