...
Purpose: Users can quickly enter workspace directories without manually finding
paths or SSH hosts. Supports both local (spawn subshell) and remote (SSH
session) workflows.
...
@src/commands/attempt.ts @src/api/config.ts @src/commands/config.ts
...
2. **Add shell key to config command** in `src/commands/config.ts`:
   ...
   - Update description string to include "shell" in available keys: "Available
     keys: api-url, shell"
   - Display shell in show command: `console.log(\`Shell: \${config.shell ||
     "(default: bash)"}\`);`
...
3. **Create isLocalhost utility** in `src/utils/localhost.ts`:
   ...
   - Return true if hostname is: "localhost", "::1", starts with "127.", or
     equals "0.0.0.0"
...
5. CD-05: Works with auto-detection from current branch </success_criteria>
