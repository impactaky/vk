...
- Pull Request (PR) creation/management via vibe-kanban API
  - Endpoints: `/task-attempts/{id}/pr`, `/task-attempts/{id}/pr/attach`, `/task-attempts/{id}/pr/comments`
  - Type definitions: `CreatePRRequest`, `PRResult`, `UnifiedPRComment` in `src/api/types.ts`
...
- Verbose mode available via `-v, --verbose` flag
  ...
  - API client logs request method, URL, body, response status, and response body when enabled
...
- GitHub Actions (`.github/workflows/release.yml`)
  ...
  - Jobs:
    ...
    2. **build**: Cross-platform binary compilation for 5 targets (Linux x86/ARM, Windows x86, macOS x86/ARM)
...
**Optional env vars:**
- `HOME` / `USERPROFILE` - For locating config directory (`~/.config/vibe-kanban/`)
...
- API calls to vibe-kanban backend endpoints:
  ...
  - `/api/task-attempts/{id}/merge`, `/push`, `/rebase`, `/rename-branch` - Git operations
