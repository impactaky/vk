...
`vk task-attempts create` MUST accept prompt text from `--description <text>`,
`--file <path>`, or an interactive editor session when neither flag is provided.
...
#### Scenario: Create with editor fallback

- **WHEN** user runs create without both `--description` and `--file`
- **THEN** CLI opens the configured editor
- **AND** uses the saved editor content as `prompt`
...
`vk task-attempts spin-off [id]` MUST accept prompt text from
`--description <text>`, `--file <path>`, or an interactive editor session when
...
#### Scenario: Spin-off with editor fallback

- **WHEN** user runs `vk task-attempts spin-off [id]` without `--description`
  and `--file`
- **THEN** CLI opens the configured editor
- **AND** uses the saved editor content as create-and-start `prompt`
...
- **WHEN** the editor is closed with only whitespace or ignored comment lines
- **THEN** CLI returns a clear validation error before API submission.
